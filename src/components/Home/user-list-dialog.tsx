	...
	const handleCreateConversation = async () => {
		...
		try {
			...
			if(!isGroup){
				...
				const postUrl = await generatedUrl() ;
			...
			<DialogContent>
				...
				{selectedUsers.length > 1 && (
					<>
						...
						/>
						<Button className='flex gap-2 bg-green-primary' onClick={()=>imgRef.current?.click()}>
							<ImageIcon size={20} />
							Group Image
						</Button>
					</>
				...
				<div className='flex justify-between'>
					...
					>
						{/* spinner */}
						{isLoading ? (
							<div className='w-5 h-5 border-t-2 border-b-2 bg-green-primary  rounded-full animate-spin' />
						) : (
							"Create"
						)}
					</Button>
